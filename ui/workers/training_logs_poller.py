...
class TrainingLogsPoller(QThread):
    logs_received = pyqtSignal(list)

    def __init__(self, api_url, user_id, session_token, train_uuid):
        super().__init__()
        ...
        while self._running:
            ...
            try:
                response = requests.get(
                    self.api_url + f"/users/{self.user_id}/trainings/{self.train_uuid}/logs",
                    headers={
