...
def add_calculated_columns(df, config):
    try:
        ...
        for col_name in config["calculated_columns"]:
            if col_name == 'typical':
...
def add_features(df, config):
    try:
        ...
        for f, col_name in zip(features, config["features"]):
...
def add_targets(df, config):
    try:
        ...
        for t, col_name in zip(targets, config["targets"]):
...
def add_diff(df, config):
    try:
        if df is None or df.empty:
            return None
        
        cols_to_diff = config["targets"]
