...
def split_data(df, samples_subset_2, selected_cols):
    try:
...
def calculate_stats(df, selected_cols):
    try:
        if df is None or df.empty:
            return None, None
...
def scale_data(df, ser_mean, ser_std, selected_cols):
    try:
        if df is None or df.empty:
            return None
...
def create_tensors(df, selected_cols, device):
    try:
        if df is None or df.empty:
            return None
        if not device:
...
def descale_data(ten_norm, ser_mean, ser_std, selected_cols):
    try:
        if ten_norm is None:
            return None
        if ser_mean is None or ser_mean.empty:
