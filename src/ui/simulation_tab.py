...
# /* Import workera z zewnętrznego pliku */
...
class SimulationTab(QWidget):
    ...
    # /* Przyjmujemy tab_widget jako argument, żeby mieć nad nim kontrolę */
    ...
    def init_ui(self):
        ...
        # Żadnego setEnabled(False) tutaj - wszystko ma być klikalne
        self.buttons = [
    ...
    # /* Blokada paska zakładek TYLKO podczas pracy */
    def toggle_ui_lock(self, is_running: bool):
        if self.tab_widget:
            self.tab_widget.tabBar().setEnabled(not is_running)
        
        # Opcjonalnie blokujemy przyciski modyfikacji, żeby nie klikać na oślep
    ...
    def on_run_simulation(self):
        ...
        # ODBLOKOWUJEMY WYJŚCIE Z ZAKŁADKI
    ...
    def on_load_params_to_fields(self):
        row = self.sim_table.currentRow()
        if row < 0: return
        try:
            samples_sim = self.sim_table.item(row, 0).data(32)
            pred_samples = self.sim_table.item(row, 1).data(32)
            full_tooltip = self.sim_table.item(row, 0).toolTip()
            strat_text = full_tooltip.replace("Strategie: ", "")
            self.param_fields["samples_simulation"].setText(str(samples_sim))
            self.param_fields["predicted_samples"].setText(str(pred_samples))
            self.param_fields["strategies"].setText(strat_text)
        except: pass
    ...
    def on_source_table_clicked(self, row, column):
        ...
        if item: self.last_clicked_training_uuid = item.text()

    def on_sim_table_clicked(self, row, column):
        item = self.sim_table.item(row, 0)
        if item: self.last_clicked_sim_uuid = item.text()

    def on_add_simulation(self):
        if not self.last_clicked_training_uuid: return
        try:
    ...
    def on_remove_simulation(self):
        if not self.last_clicked_sim_uuid: return
        try:
    ...
    def fill_sim_table(self):
        try:
            selected_uuid = self.last_clicked_sim_uuid
            ...
            for row, sim in enumerate(simulations):
                curr = str(sim.get("sim_uuid", ""))
                if selected_uuid and curr == selected_uuid: target_row = row
                item_uuid = QTableWidgetItem(curr)
                item_uuid.setData(32, sim.get("samples_simulation"))
                self.sim_table.setItem(row, 0, item_uuid)
                item_train = QTableWidgetItem(str(sim.get("training_job_uuid", "")))
                item_train.setData(32, sim.get("predicted_samples"))
                self.sim_table.setItem(row, 1, item_train)
            ...
            if target_row != -1: self.sim_table.selectRow(target_row)
        except: pass
